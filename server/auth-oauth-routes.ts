...
setInterval(
  () => {
    ...
    if (deletedStateCount > 0) {
      console.log(
        `🧹 [STATE-CLEANUP] Removed ${deletedStateCount} expired OAuth states`,
      );
    }
    ...
    if (deletedExchangeCount > 0) {
      console.log(
        `🧹 [EXCHANGE-CLEANUP] Removed ${deletedExchangeCount} expired session exchange codes`,
      );
    }
  },
  5 * 60 * 1000,
...
export async function createGoogleOAuthURLRoute(req: Request, res: Response) {
  try {
    ...
    if (isDevelopment) {
      ...
      console.log(
        "⚠️ [OAUTH-URL] Unknown domain, using current host as fallback",
      );
      redirectUri = `https://${host}/api/auth/google/callback`;
    }
 ...
 */
export async function handleGoogleOAuthCallbackRoute(
  req: Request,
  res: Response,
) {
  try {
    ...
    if (error) {
      ...
      if (error === "access_denied") {
        errorMessage =
          "Authentication was cancelled. You need to grant permission to continue.";
      } else if (error === "invalid_request") {
        errorMessage = "Authentication request was invalid. Please try again.";
      }

      return res.redirect(
        `/auth?error=oauth_error&message=${encodeURIComponent(errorMessage)}&canRetry=true`,
      );
    }

    if (!code || !state) {
      console.error(
        "❌ [OAUTH-CALLBACK-ERROR] Missing authorization code or state:",
        {
          hasCode: !!code,
          hasState: !!state,
          query: req.query,
      ...
      return res.redirect(
        "/auth?error=missing_params&message=Authentication%20response%20incomplete.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (!stateData) {
      console.error(
        "❌ [STATE-VALIDATION] Invalid or missing state parameter:",
        {
          providedState: state
            ? `${(state as string).substring(0, 10)}...`
            : "null",
          storeHasStates: stateStore.size > 0,
          allStates: Array.from(stateStore.keys()).map(
            (k) => k.substring(0, 10) + "...",
          ),
        },
      );
      return res.redirect(
        "/auth?error=invalid_state&message=Authentication%20session%20invalid.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (stateData.timestamp < fifteenMinutesAgo) {
      console.error("❌ [STATE-VALIDATION] Expired state parameter:", {
        ...
        expiredBy:
          Math.floor((fifteenMinutesAgo - stateData.timestamp) / 1000) +
          " seconds",
      });
      stateStore.delete(state as string);
      return res.redirect(
        "/auth?error=expired_state&message=Authentication%20session%20expired.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (isDevelopment) {
      ...
      // Fallback for unknown domains - use current host if HTTPS
      console.log(
        "⚠️ [OAUTH-CALLBACK] Unknown domain, using current host as fallback",
      );
      redirectUri = `https://${host}/api/auth/google/callback`;
    }
    ...
    if (!tokenResponse.ok) {
      ...
      let errorMessage =
        "Authentication failed during token exchange. Please try again.";
      ...
      return res.redirect(
        `/auth?error=token_exchange_failed&message=${encodeURIComponent(errorMessage)}&canRetry=true`,
      );
    }
    ...
    if (!userResponse.ok) {
      ...
      if (userResponse.status === 401) {
        ...
        errorMessage =
          "Insufficient permissions to access profile. Please try signing in again.";
      }

      return res.redirect(
        `/auth?error=user_info_failed&message=${encodeURIComponent(errorMessage)}&canRetry=true`,
      );
    }
    ...
    console.log(
      "🔍 [AUTH-FIX] Looking up user by Google ID first:",
      userData.googleId,
    );
    ...
    if (existingUser) {
      ...
      console.log(
        "🔍 [AUTH-FIX] No user found by Google ID, checking by email as fallback",
      );
      ...
      if (userByEmail) {
        ...
        if (user && user.id) {
          try {
            console.log(
              `🎯 [STARTER-QUESTS] Assigning starter quests to new user ${user.id} (${user.name})`,
            );

            // Assign both career and social starter quests
            const starterCareerQuests = await storage.assignDailyQuestsToUser(
              user.id,
            );
            const starterSocialQuests = await storage.assignDailySocialQuests(
              user.id,
            );

            const totalStarterQuests =
              starterCareerQuests.length + starterSocialQuests.length;
            console.log(
              `✅ [STARTER-QUESTS] Successfully assigned ${starterCareerQuests.length} career + ${starterSocialQuests.length} social = ${totalStarterQuests} starter quests to new user ${user.name}`,
            );
          } catch (starterError) {
            console.error(
              `❌ [STARTER-QUESTS] Error assigning starter quests to new user ${user.id}:`,
              starterError,
            );
    ...
    try {
      ...
      console.log(
        "⚠y � [OAUTH CALLBACK] Could not parse state for return host, using current host",
      );
    }
    ...
    if (needsCrossDomainHandoff) {
      ...
      console.log(
        "✅ [SESSION-HANDOFF] Generated exchange code and redirecting to:",
        sessionAcceptUrl,
      );
      ...
      console.log(
        "🍪 [SESSION-HANDOFF] Same domain - setting cookie directly:",
        {
      ...
      // Serve a small HTML that notifies the opener and closes the popup
      return res.send(`
        <html>
          <head>
                ...
                try {
                  ...
                  if (window.opener) {
                   ...
                   window.close();
                    ...
                    // );
                    // Optionally, you can directly redirect parent:
                    // window.opener.location = "/dashboard";
                  }
                } catch (err) {
    ...
    console.error(
      "❌ [OAUTH-CALLBACK-CRITICAL] OAuth callback critical error:",
      {
        errorMessage: error.message,
        errorStack: error.stack,
        host: req.get("host"),
    ...
    if (
      ...
      userMessage =
        "Google authentication service error. Please try again later.";
    }

    res.redirect(
      `/auth?error=callback_error&message=${encodeURIComponent(userMessage)}&canRetry=true&errorCode=OAUTH_CALLBACK_ERROR`,
    );
  }
}
...
export async function acceptSessionRoute(req: Request, res: Response) {
  try {
    ...
    if (!code || typeof code !== "string") {
      ...
      return res.redirect(
        "/auth?error=invalid_exchange_code&message=Authentication%20session%20code%20invalid.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (!exchangeData) {
      console.error(
        "❌ [SESSION-ACCEPT] Exchange code not found or already used:",
        {
          codeProvided: code.substring(0, 10) + "...",
          storeSize: sessionExchangeStore.size,
          allCodes: Array.from(sessionExchangeStore.keys()).map(
            (k) => k.substring(0, 10) + "...",
          ),
        },
      );
      return res.redirect(
        "/auth?error=exchange_code_not_found&message=Authentication%20session%20not%20found%20or%20already%20used.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (exchangeData.timestamp < fiveMinutesAgo) {
      console.error("❌ [SESSION-ACCEPT] Exchange code expired:", {
        codeAge:
          Math.floor((Date.now() - exchangeData.timestamp) / 1000) + " seconds",
        maxAge: "300 seconds (5 minutes)",
      });
      sessionExchangeStore.delete(code);
      return res.redirect(
        "/auth?error=exchange_code_expired&message=Authentication%20session%20expired.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }
    ...
    if (exchangeData.returnHost !== currentHost) {
      ...
      return res.redirect(
        "/auth?error=host_mismatch&message=Authentication%20domain%20mismatch.%20Please%20try%20signing%20in%20again.&canRetry=true",
      );
    }

    console.log(
      "✅ [SESSION-ACCEPT] Exchange code valid, setting session cookie",
    );
    ...
    console.error(
      "❌ [SESSION-ACCEPT-ERROR] Session acceptance critical error:",
      {
        errorMessage: error.message,
        errorStack: error.stack,
        host: req.get("host"),
    ...
    let userMessage =
      "Failed to complete authentication session. Please try signing in again.";
    if (error.message?.includes("cookie")) {
      userMessage =
        "Unable to set authentication cookie. Please enable cookies and try again.";
    } else if (error.message?.includes("domain")) {
      userMessage = "Authentication domain error. Please try signing in again.";
    }

    res.redirect(
      `/auth?error=session_accept_error&message=${encodeURIComponent(userMessage)}&canRetry=true&errorCode=SESSION_ACCEPT_ERROR`,
    );
  }
}
...
export async function checkSessionRoute(req: Request, res: Response) {
  try {
    ...
    if (!sessionToken) {
      console.log(
        "❌ [SESSION-DEBUG] No session cookie found - detailed analysis:",
        {
          cookiePresent: !!sessionCookie,
          ...
          possibleCookieIssues: [
            ...
            cookieHeader && !sessionCookie
              ? "Cookie header present but session cookie missing"
              : null,
            sessionCookie && typeof sessionCookie !== "string"
              ? "Session cookie wrong type"
              : null,
          ].filter(Boolean),
          suggestedFixes: [
    ...
    try {
      ...
      if (decoded.userId) {
        console.log(
          "🔍 [SESSION-CHECK] Looking up user by ID:",
          decoded.userId,
        );
      ...
      if (!user) {
        console.log(
          "🔍 [SESSION-CHECK] Fallback: Looking up user by email:",
          decoded.email,
        );
      ...
      if (!user) {
        console.log(
          "❌ [SESSION-CHECK] User not found in database with ID or email",
        );
        return res.status(401).json({
          success: false,
      ...
      if (user.photoURL) {
        if (user.photoURL.startsWith("data:image/")) {
          ...
          console.log(
            "✅ [SESSION-CHECK] Using custom uploaded profile picture",
          );
          ...
          console.log(
            "⚠️ [SESSION-CHECK] Unknown photoURL format:",
