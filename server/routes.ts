...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Perplexity - Career Advice
  apiRouter.post("/perplexity/career-advice", async (req: Request, res: Response) => {
    try {
      // Check if PERPLEXITY_API_KEY is present
      if (!process.env.PERPLEXITY_API_KEY) {
        return res.status(503).json({ 
          message: "Perplexity API service unavailable. API key is missing.",
          requiresApiKey: true
        });
      }
      ...
      const { getCareerAdvice } = await import('./services/perplexity-service');
      ...
      console.error("Error generating career advice with Perplexity:", error);
      res.status(500).json({ message: "Error generating career advice" });
    }
  });

  // Perplexity - Resume Analysis
  apiRouter.post("/perplexity/analyze-resume", async (req: Request, res: Response) => {
    try {
      // Check if PERPLEXITY_API_KEY is present
      if (!process.env.PERPLEXITY_API_KEY) {
        return res.status(503).json({ 
      ...
      // Import Perplexity service
      const { analyzeResume } = await import('./services/perplexity-service');
  ...
  // Perplexity - Networking Recommendations
  apiRouter.post("/perplexity/networking-recommendations", async (req: Request, res: Response) => {
    try {
      // Check if PERPLEXITY_API_KEY is present
      if (!process.env.PERPLEXITY_API_KEY) {
        return res.status(503).json({ 
      ...
      // Import Perplexity service
      const { generateNetworkingRecommendations } = await import('./services/perplexity-service');
      ...
      console.error("Error generating networking recommendations with Perplexity:", error);
      res.status(500).json({ message: "Error generating networking recommendations" });
    }
  });
  
  // Perplexity - Skill Gap Analysis
  apiRouter.post("/perplexity/skill-gaps", async (req: Request, res: Response) => {
    try {
      // Check if PERPLEXITY_API_KEY is present
      if (!process.env.PERPLEXITY_API_KEY) {
        return res.status(503).json({ 
      ...
      const { userId, targetJobTitle, targetIndustry } = req.body;
      
      if (!userId || !targetJobTitle || !targetIndustry) {
        return res.status(400).json({ 
          message: "User ID, target job title, and target industry are required" 
        });
      }
      ...
      // Import Perplexity service
      const { identifySkillGaps } = await import('./services/perplexity-service');
      
      // Identify skill gaps
      const skillGapAnalysis = await identifySkillGaps(
        skills,
        targetJobTitle,
        targetIndustry
      );
      
      // Save the analysis as a chat message
      await storage.createChatMessage({
        userId,
        sender: "perplexity",
        content: skillGapAnalysis,
        messageType: "skill_gap_analysis"
      });
      
      res.json({ skillGapAnalysis });
    } catch (error) {
      console.error("Error identifying skill gaps with Perplexity:", error);
      res.status(500).json({ message: "Error identifying skill gaps" });
    }
  });
