...
export async function updateQuestProgress(questId: number, userId: number, progress: number): Promise<any | null> {
  try {
    ...
    if (result.rows.length === 0) {
      console.log(`Quest ${questId} not found for user ${userId}`);
      return null;
    }
    ...
    if (defResult.rows.length === 0) {
      console.log(`Quest definition ${questDefinitionId} not found`);
      return null;
    }
    ...
    if (updateResult.rows.length === 0) {
      console.log(`Failed to update quest ${questId} for user ${userId}`);
      return null;
    }
