...
import { enhancedIntentClassifier, EnhancedClassificationResult } from './enhanced-intent-classifier';
import { conversationGoalTrackerService } from './conversation-goal-tracker';
import { generateFollowUpIntelligence } from './musk-followup-intelligence';

/**
 * Process user message with perfected enhanced intelligence system
 */
export async function processEnhancedMuskRequest(request: EnhancedMuskRequest): Promise<EnhancedMuskResponse> {
  try {
    console.log('[Perfect Musk] Processing request for user:', request.userId);
    const userIdString = request.userId.toString();

    // 1. Unified Intent + Emotion + Stage Detection
    const classification: EnhancedClassificationResult = enhancedIntentClassifier.classify(request.message, request.userProfile);
    console.log(`[Perfect Musk] Result: Intent=${classification.intent}, Emotion=${classification.emotion}, Stage=${classification.stage}`);

    // 2. Refresh Memory & Guidelines
    ...
    // Check if we have an active goal for this session
    let goalSuggestion = '';
    try {
      // For now we use userId as a proxy for conversationId or look it up
      const existingGoals = await pool.query(
        'SELECT conversation_id FROM conversation_goals WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1',
        [request.userId]
      );

      if (existingGoals.rows.length > 0) {
        const goalId = existingGoals.rows[0].conversation_id;
        const currentGoal = await conversationGoalTrackerService.getConversationGoal(request.userId, goalId);

        if (currentGoal) {
          // Check if message completes a sub-goal (simple heuristic)
          for (const subGoal of currentGoal.subGoals) {
            if (!subGoal.completed && request.message.toLowerCase().includes(subGoal.text.toLowerCase())) {
              await conversationGoalTrackerService.completeSubGoal(currentGoal.id!, subGoal.text);
              console.log(`[Perfect Musk] Sub-goal completed: ${subGoal.text}`);
            }
          }

          const nextStep = conversationGoalTrackerService.generateNextFollowUp(currentGoal);
          goalSuggestion = nextStep.text;
        }
      } else {
        // Create initial goal if none exists
        await conversationGoalTrackerService.createConversationGoal(
          request.userId,
          request.message,
      ...
      console.warn('[Perfect Musk] Goal tracking error:', goalError);
    }
    ...
    // 6. Hybrid Knowledge Engine (Retrieval + Reasoning)
    const filResult = await generateFollowUpIntelligence(
      {
        userId: request.userId,
        userData: request.userProfile,
        experiences: request.userExperiences,
        skills: request.userSkills,
        projects: request.userProjects,
      },
      classification.intent,
      request.conversationHistory as any
    );

    // 7. Structured Multi-turn Planning
    const emotionalContext = analyzeEmotionalContext(userIdString, processedMessage);
    const emotionalStrategy = generateEmotionalResponseStrategy(emotionalContext, request.userProfile);
    const predictiveInsights = generatePredictiveInsights(userIdString, request.userProfile, request.userExperiences, request.userSkills);
    const crossUserRecs = await generateCrossUserRecommendations(userIdString, request.userProfile, userPatterns);

    // 8. Final Intelligence Generation (Unified Prompt)
    ...
    const response = await generatePhase3EnhancedResponse(
      ...
      conversationContext,
      personaAnalysis,
      { ...classification, conversationStage: classification.stage },
      personalizedGuidelines,
      { suggestions: filResult.followUps.map(fu => ({ title: fu.text, description: fu.purpose, actionText: fu.text })) } as any,
      predictiveInsights,
      crossUserRecs,
      emotionalContext,
      emotionalStrategy,
    ...
    // Append Quick Response Options from FIL and Goal Tracker
    const quickResponses = [...filResult.followUps.map(f => f.text)];
    if (goalSuggestion && !quickResponses.includes(goalSuggestion)) {
      quickResponses.push(goalSuggestion);
    }

    const finalResponse = `${response}\n\nQuick Response Options: ${quickResponses.map(q => `"${q}"`).join(', ')}`;

    // Add to memory
    addMessageToMemorySync(userIdString, 'musk', finalResponse, personaAnalysis.selectedPersona.name);
    ...
    return {
      ...
      metadata: {
        ...
        proactiveSuggestions: quickResponses,
        contextUsed: {
          profileCompleteness: enrichedContext.user.profileCompleteness.score,
          keyInsights: extractKeyInsights(enrichedContext),
          recommendedActions: quickResponses,
          journeyStage: classification.stage,
          emotionalState: classification.emotion
        }
      }
    };

  } catch (error) {
    console.error('[Perfect Musk] Critical Error:', error);
    const fallbackResponse = await generateFallbackResponse(request.message, request.userProfile);
    return {
      response: fallbackResponse,
      metadata: {
        intent: 'general_advice' as any,
        persona: 'generalist',
        confidence: 0.5,
        proactiveSuggestions: [],
        contextUsed: { profileCompleteness: 0, keyInsights: [], recommendedActions: [] }
      }
    };
  }
}
...
async function generateContextualResponse(context: EnrichedContext, message: string, conversationContext?: string): Promise<string> {
  ...
  try {
    ...
    // Check if asking about OTHER platforms (not Brandentify)
    ...
    // Check if asking about Brandentify specifically
    const isBrandentifyQuestion = messageLower.includes('brandentifier') &&
    ...
    if (isNetworkingQuestion) {
      ...
      if (isOtherPlatformsQuestion) {
        // Question about OTHER platforms besides Brandentify
...
` : ''}The user is asking about networking on platforms OTHER than Brandentify. Provide a balanced response that:
...
4. Still positions Brandentify as a comprehensive solution
5. Gives specific advice for their industry (${industry}) and role (${title})

Be honest about other platforms while highlighting Brandentify's unique value proposition.`;
      } else if (isBrandentifyQuestion) {
        // Question specifically about Brandentify features
        networkingPrompt = `Answer this Brandentify networking question for ${userName}, a ${title} in ${industry}: "${message}"
...
` : ''}The user is asking specifically about Brandentify's networking features. Provide detailed guidance on:
1. How to use Brandentify's networking features effectively
2. Specific features available on the platform
3. Step-by-step instructions for their industry (${industry})
4. Tips for maximizing networking success on Brandentify
5. Industry-specific networking strategies for ${title} professionals

Focus entirely on Brandentify's capabilities and how to leverage them.`;
      } else {
...
2. Includes both Brandentify and other platform strategies
...
async function generateIntelligentResponse(prompt: string, context: EnrichedContext, message: string = '', userId?: string): Promise<string> {
  console.log('[Perfect Musk] Using fully constructed enhanced prompt for maximum fidelity');

  try {
    const aiResult = await generateAIResponse(prompt, {
      systemPrompt: "You are the Musk AI Career Strategist. Structure every response using Conversational Strategy Blocks: 1. Primary Insight (Direct & Bold), 2. Strategic Reasoning (Why this matters), 3. Immediate Next Step (Practical action).",
      maxTokens: 600,
      temperature: 0.7,
    });

    const aiAdvice = aiResult.text || "";
    const industry = context.user.basicInfo.industry || 'your field';
    const cta = `\n\n**Next Steps on Brandentify:**\n• Complete your profile for 360° visibility\n• Showcase your ${industry} expertise in your portfolio`;

    return `${aiAdvice}${aiAdvice.includes('Brandentify') ? '' : cta}`;
  } catch (error) {
    console.error('[Perfect Musk] AI Generation Error:', error);
...
Keep response under 500 words with clear, actionable recommendations based on their actual profile.`;
    ...
    const aiResult = await generateAIResponse(careerPrompt, {
      maxTokens: 400,
      temperature: 0.7,
    });
...
async function generateAdvancedFallback(context: EnrichedContext, message: string): Promise<string> {
  ...
  if (isNetworking) {
    ...
    // Determine if asking about other platforms vs Brandentify specifically
    ...
    const isBrandentifyQuestion = /brandentifier/i.test(message) && !isOtherPlatformQuestion;

    console.log(`[Enhanced Musk] Platform analysis: isOtherPlatform=${isOtherPlatformQuestion}, isBrandentify=${isBrandentifyQuestion}`);
...
The user is asking about networking on platforms OTHER than Brandentify. Provide a balanced, honest response that:
1. Acknowledges that yes, they can network on other platforms
...
5. Still mentions Brandentify as one comprehensive option among others

Be honest about other platforms while positioning Brandentify appropriately.`;
      } else if (isBrandentifyQuestion) {
        console.log(`[Enhanced Musk] Generating response for BRANDENTIFY-specific question`);
        networkingPrompt = `Answer this Brandentify networking question for ${userName}, a ${title} in ${industry}: "${message}"

The user is asking specifically about Brandentify's networking features. Focus on:
1. Detailed Brandentify networking capabilities
...
Focus entirely on Brandentify's features and capabilities.`;
      } else {
...
3. Mentions relevant platforms including Brandentify
...
Make the response specific to their question and professional context.`;
      ...
      const aiResult = await generateAIResponse(networkingPrompt, {
        maxTokens: 400,
        temperature: 0.7,
      });

      const networkingAdvice = aiResult.text;
      ...
      response += `• Brandentify: Comprehensive platform for career development and networking\n`;
...
5. Prioritizes Brandentify platform usage for networking and professional growth
...
**Immediate Actions on Brandentify:**
...
What specific career challenge or opportunity would you like to discuss further?`;
    ...
    response += `**Immediate Actions on Brandentify:**\n`;
...
Great question about Brandentify's networking features! As a ${title} in ${industry}, here's how to leverage our platform's networking capabilities effectively:

**Brandentify Networking Features:**
