...
async function generateContextualFallback(context: EnrichedContext, currentMessage: string = ''): Promise<string> {
  ...
  const isCareerSpecificQuestion = currentMessage.toLowerCase().includes('portfolio') ||
                                   currentMessage.toLowerCase().includes('skills') ||
  ...
  // For profile enhancement questions, provide specific advice
  if (isProfileQuestion && !isCareerSpecificQuestion) {
    return `${userName}, to make your profile more compelling as a ${title} in ${industry}:
...
Use action verbs like "Led," "Transformed," "Optimized," and "Delivered" to emphasize leadership.`;
  ...
  // For all other questions, provide general career guidance
  return generateBasicFallback(currentMessage, context.user.basicInfo);
}
