...
/**
 * Generate AI-powered INITIAL suggestions when user starts chat (no conversation history)
 */
export const handleGenerateInitialSuggestions = async (req: Request, res: Response) => {
  try {
    const { userId: rawUserId } = req.body;
    
    console.log('[Initial Suggestions] Generating personalized welcome questions');
    ...
    if (!userId) {
      // Return generic welcome questions if no user
      return res.json({ 
        suggestions: [
    ...
    try {
      ...
      if (global.resumeContexts && global.resumeContexts[userId]) {
        ...
        resumeHighlights = resumeContext.resumeText?.substring(0, 400) || '';
      }
      
      console.log(`[Initial Suggestions] Loaded rich data: ${userExperiences.length} experiences, ${userSkills.length} skills, ${userEducations.length} education, ${userProjects.length} projects`);
      
    } catch (error) {
      console.error('[Initial Suggestions] Error fetching user data:', error);
    }
...
- Location: ${userData?.location || 'Not specified'}`;
    ...
    // Create enhanced AI prompt for generating personalized WELCOME/INITIAL questions
    const prompt = `You are Musk, an AI career expert. Generate 4 HIGHLY PERSONALIZED welcome questions for a user who just opened the chat for the first time.

${enrichedProfile}

This is their FIRST interaction - NO conversation history yet. Generate 4 personalized welcome questions that show you understand their profile.

CRITICAL PERSONALIZATION RULES:
1. Use SPECIFIC details from their profile (companies, skills, education, projects, years of experience)
2. Questions should reflect their career stage (${careerStage}-level professionals have different priorities)
3. Reference their ACTUAL background - e.g., "With your 5 years at Google..." or "Your React + Python combo..."
4. Make questions ACTIONABLE and immediately valuable
5. Consider their industry (${userData?.industry || 'their field'}) norms and typical progression
6. Match question complexity to career stage (entry=foundations, senior=strategy/leadership)
7. NEVER ask generic questions like "What skills should I learn?" - be SPECIFIC to their profile
8. NEVER suggest "Analyze my resume" or "Evaluate my pitch deck" - there are upload buttons for these
9. Focus on HIGH-IMPACT career topics that matter most for their stage and goals
10. Make them feel like you've carefully reviewed their entire profile

Career Stage-Specific Focus:
- Entry-level: Skill building, first career moves, foundation certifications
- Mid-level: Specialization vs. breadth, promotion readiness, leadership transition
- Senior-level: Executive positioning, thought leadership, mentoring, strategic influence
- Executive: C-suite moves, board roles, advisory work, industry leadership

Examples of EXCELLENT personalized welcome questions:
- "With your React expertise and 3 years at startup X, should you target Staff Engineer at FAANG or Tech Lead at high-growth Series B?"
- "Your finance + MBA background positions you perfectly for FinTech PM roles - should we map out that transition strategy?"
- "As a ${careerStage} ${userData?.title || 'professional'} in ${userData?.industry || 'your field'}, what's your biggest career challenge right now?"

Examples of BAD generic questions:
- "How can I help you today?" (too vague, no personalization)
- "What are your career goals?" (doesn't use their data)
- "What skills should I learn?" (generic, not tailored)

Format: Exactly 4 questions, one per line, no numbering, no bullets. Each question should make the user think "Wow, this AI actually understands my career!"`;

    try {
      // Use VPS Ollama for FREE AI generation (same as main Musk chat)
      console.log('[Initial Suggestions] Calling VPS Ollama for personalized welcome questions...');
      ...
      if (ollamaResponse.ok) {
        ...
        const questions = generatedText
          ...
          .filter((q: string) => {
            ...
            // Filter out resume and pitch deck upload suggestions
            ...
            if (lowerQ.includes('upload')) return false;
            return true;
          })
          .slice(0, 4);
        
        if (questions.length >= 3) {
          console.log('[Initial Suggestions] Successfully generated AI welcome questions:', questions.length);
          return res.json({ 
            suggestions: questions,
            source: 'ai-ollama-initial'
          });
        }
      }
    } catch (ollamaError) {
      console.log('[Initial Suggestions] VPS Ollama not available, using enriched fallback');
    }
    
    // Fallback to enriched profile-aware welcome questions if AI fails
    console.log('[Initial Suggestions] Using personalized fallback welcome questions');
    const fallbackQuestions = generateInitialFallbackSuggestions(
      userData, 
      userExperiences, 
      userSkills, 
      userEducations, 
    ...
    return res.json({ 
      ...
      source: 'fallback-initial'
    });
    
  } catch (error) {
    console.error('[Initial Suggestions] Error:', error);
    
    // Return basic welcome questions
    return res.json({ 
      suggestions: [
        'What career advice can you offer?',
        'How can I improve my professional profile?',
      ...
      source: 'error-fallback-initial'
    });
  }
};

/**
 * Generate enriched fallback for INITIAL welcome suggestions using actual user career data
 */
function generateInitialFallbackSuggestions(
  userData: any,
  experiences: any[],
  skills: any[],
  educations: any[],
  ...
  // PERSONALIZED welcome questions based on career stage + actual data
  if (careerStage === 'entry' && totalYears < 3) {
    suggestions.push(`With ${degree} in ${field} and ${topSkill} skills, what's the fastest path to mid-level roles in ${industry}?`);
    suggestions.push(`As an entry-level ${latestRole}, which certifications would accelerate your career most?`);
    suggestions.push(`How can you leverage ${latestCompany} experience to land your next role?`);
    suggestions.push(`Should you specialize in ${topSkill} or build T-shaped skills for ${industry}?`);
  } else if (careerStage === 'mid' && totalYears >= 3 && totalYears < 7) {
    suggestions.push(`After ${Math.round(totalYears)} years as ${latestRole}, should you target Staff/Lead or pivot to management?`);
    suggestions.push(`With ${topSkill}${secondSkill ? ' and ' + secondSkill : ''} expertise, what's your optimal next move in ${industry}?`);
    suggestions.push(`How do you transition from ${latestRole} at ${latestCompany} to senior leadership?`);
    suggestions.push(`Is your ${latestCompany} experience enough to break into FAANG/top-tier companies?`);
  } else if (careerStage === 'senior' && totalYears >= 7 && totalYears < 12) {
    suggestions.push(`With ${Math.round(totalYears)} years in ${industry}, how do you position for Director/VP roles?`);
    suggestions.push(`As a Senior ${latestRole}, should you specialize deeper or build cross-functional leadership?`);
    suggestions.push(`How can you leverage ${latestCompany} experience for executive-level opportunities?`);
    suggestions.push(`What thought leadership strategies work best for ${careerStage} ${industry} professionals?`);
  } else if (careerStage === 'executive') {
    suggestions.push(`With ${Math.round(totalYears)} years experience, what's your path to C-suite or board roles?`);
    suggestions.push(`Should you leverage ${industry} expertise for consulting, advisory, or executive positioning?`);
    suggestions.push(`How can you build industry thought leadership using ${topSkill} and ${latestCompany} credibility?`);
    suggestions.push(`What's the strategic move from ${latestRole} to Chief ${industry.includes('Tech') ? 'Technology' : industry} Officer?`);
  }
  
  // Profile goal-specific suggestions
  if (lookingFor.includes('job') && suggestions.length < 4) {
    suggestions.push(`Based on ${latestRole} at ${latestCompany}, what companies should you target next?`);
  } else if (lookingFor.includes('mentor') && suggestions.length < 4) {
    suggestions.push(`Who in ${industry} would be ideal mentors for your ${careerStage}-level career goals?`);
  } else if (lookingFor.includes('network') && suggestions.length < 4) {
    suggestions.push(`How can you leverage ${topSkill} expertise to attract meaningful ${industry} connections?`);
  }
  
  // Fill with career-stage appropriate defaults if needed
  while (suggestions.length < 4) {
    if (careerStage === 'entry') {
      suggestions.push(`What ${industry} companies value candidates with ${field} backgrounds?`);
    } else if (careerStage === 'mid') {
      suggestions.push(`Should you deepen ${industry} expertise or pivot to adjacent fields?`);
    } else {
      suggestions.push(`What's your biggest career challenge as a ${careerStage} ${title}?`);
    }
  }
