...
// Handle Musk AI improve requests - enhances previous responses
export const handleMuskImprove = async (req: Request, res: Response) => {
  try {
    const { userId: rawUserId, originalMessage, context } = req.body;
    
    console.log(`Musk improve: Received improve request with rawUserId: ${rawUserId}`);
    ...
    const improvePrompt = `Please provide an enhanced and more detailed version of your previous response. 
    
Original response: "${originalMessage}"

Please:
- Add more specific details and actionable advice
- Include concrete examples where relevant
- Provide deeper insights and analysis
- Make the response more comprehensive and valuable
- Keep the same friendly, professional tone`;
    ...
    if (userId) {
      try {
        console.log('[Musk Improve] Attempting enhanced persona response');
        ...
        console.error('[Musk Improve] Enhanced system failed:', error);
        response = await generateMuskResponse(improvePrompt, enrichedContext);
      }
    } else {
      response = await generateMuskResponse(improvePrompt, enrichedContext);
    }
    ...
    if (userId) {
      const userIdString = userId.toString();
      if (userIdString) {
        updateUserInteractionMemory(userIdString, improvePrompt, response, enrichedContext);
      }
    }
    ...
    console.error("Error in Musk improve handler:", error);
    if (!res.headersSent) {
      return res.status(500).json({ error: "Failed to process improve request" });
    }
  }
};
