...
router.post('/career-goals/:goalId/regenerate-milestones', async (req, res) => {
  try {
    const goalId = parseInt(req.params.goalId);
    if (isNaN(goalId)) {
      return res.status(400).json({ message: 'Invalid goal ID' });
    ...
    // Get the capsule to check if it exists and get its information
    const capsule = await storage.getCareerCapsuleById(goalId);
    if (!capsule) {
      return res.status(404).json({ error: 'Career goal not found' });
    }

    console.log(`[Milestone Regeneration] Starting regeneration for capsule ${goalId}: ${capsule.title}`);
    console.log(`[Milestone Regeneration] Capsule details:`, JSON.stringify(capsule));

    // First delete all existing years and tasks
    console.log(`[Milestone Regeneration] Deleting existing milestones for capsule ${goalId}`);
    
    // Implementation of deleting years and tasks directly in the endpoint
    // since we're having issues with adding to storage.ts
    try {
      // Find all years for this capsule
      const years = await storage.getCapsuleYearsByCapsuleId(goalId);
      console.log(`[Milestone Regeneration] Found ${years.length} years to delete`);
      
      // Delete all tasks for these years
      for (const year of years) {
        const tasks = await storage.getCapsuleTasksByYearId(year.id);
        console.log(`[Milestone Regeneration] Deleting ${tasks.length} tasks for year ${year.id}`);
        
        // Delete each task
        for (const task of tasks) {
          await storage.deleteCapsuleTask(task.id);
        }
        
        // Delete the year
        await storage.deleteCapsuleYear(year.id);
      }
      
      console.log(`[Milestone Regeneration] Successfully deleted all existing milestones`);
    } catch (error) {
      console.error(`[Milestone Regeneration] Error deleting existing milestones:`, error);
      return res.status(500).json({ error: 'Failed to delete existing milestones' });
    }
    
    // Now generate new milestones
    // Check available API keys
    const openaiKey = process.env.OPENAI_API_KEY;
    const anthropicKey = process.env.ANTHROPIC_API_KEY;
    console.log(`[Milestone Regeneration] API Keys available: OpenAI: ${openaiKey ? 'YES' : 'NO'}, Anthropic: ${anthropicKey ? 'YES' : 'NO'}`);
    
    // Use Anthropic if OpenAI key is not available but Anthropic is
    const aiModel = openaiKey ? 'openai' : (anthropicKey ? 'anthropic' : 'openai');
    console.log(`[Milestone Regeneration] Selected AI model: ${aiModel}`);
    ...
    const options = {
      ...
      timeframe: parseInt(capsule.timeframe.toString()) || 3,
      industry: capsule.industry,
      description: capsule.description,
      useModel: aiModel as 'openai' | 'anthropic', // Force type to be correct
    };
    
    console.log(`[Milestone Regeneration] Generating new AI milestones with options:`, JSON.stringify(options));
    ...
    try {
      ...
      if (result.success && result.years) {
        console.log(`[Milestone Regeneration] Successfully generated ${result.years.length} years of milestones with Musk AI`);
        
        // Save the AI-generated milestones
        const saved = await saveCapsuleMilestones(goalId, result.years);
        
        if (saved) {
          // Update the capsule to indicate AI milestones were generated
          await storage.updateCareerCapsule(goalId, {
            isMuskGenerated: true,
            updatedAt: new Date()
          });
          ...
          return res.status(200).json({
            success: true,
            message: 'Successfully regenerated milestones with AI',
            milestoneCount: result.years.length
          });
        } else {
          console.error('[Milestone Regeneration] Failed to save AI-generated milestones');
          return res.status(500).json({ error: 'Failed to save regenerated milestones' });
        }
      } else {
        console.error('[Milestone Regeneration] Failed to generate AI milestones:', result.message);
        return res.status(500).json({ error: 'Failed to generate AI milestones', details: result.message });
      }
    } catch (error) {
      console.error('[Milestone Regeneration] Error during AI milestone generation:', error);
      return res.status(500).json({ error: 'Error during milestone regeneration' });
    }
  } catch (error) {
    console.error('Error regenerating milestones:', error);
    return res.status(500).json({ message: 'Error regenerating milestones' });
  }
});
