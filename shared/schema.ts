import { pgTable, text, serial, integer, boolean, timestamp, jsonb, varchar, decimal, unique, check } from "drizzle-orm/pg-core"; 
...
// Smart Connect Feature - Connection Requests
...
// Connection requests table - tracks professional connection requests between users
export const connectionRequests = pgTable("connection_requests", {
  id: serial("id").primaryKey(),
  requesterId: integer("requester_id").references(() => users.id).notNull(), // User sending the request
  recipientId: integer("recipient_id").references(() => users.id).notNull(), // User receiving the request
  message: text("message"), // Custom message from requester (max 500 chars)
  requestReason: text("request_reason"), // Why they want to connect (for analytics)
  matchScore: decimal("match_score", { precision: 3, scale: 2 }), // AI-calculated compatibility score
  sharedInterests: jsonb("shared_interests").default('[]'), // Array of shared interests/topics
  mutualConnections: integer("mutual_connections").default(0), // Number of mutual connections
  status: connectionStatusEnum("status").notNull().default("pending"),
  requestedAt: timestamp("requested_at").defaultNow(),
  respondedAt: timestamp("responded_at"), // When the request was accepted/declined
  responseMessage: text("response_message"), // Optional response message
  connectionEstablishedAt: timestamp("connection_established_at"), // When connection was established
  isStarred: boolean("is_starred").default(false), // Whether requester starred this connection
  lastInteractionAt: timestamp("last_interaction_at"), // Last time they interacted
  metadataJson: jsonb("metadata_json").default('{}'), // Additional metadata from recommendation engine
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// User connections table - tracks established connections between users
export const userConnections = pgTable("user_connections", {
  id: serial("id").primaryKey(),
  user1Id: integer("user1_id").references(() => users.id).notNull(), // First user in connection
  user2Id: integer("user2_id").references(() => users.id).notNull(), // Second user in connection
  connectionRequestId: integer("connection_request_id").references(() => connectionRequests.id), // Original request that created this connection
  connectionType: text("connection_type").default("professional"), // professional, personal, mentor, etc.
  connectionStrength: integer("connection_strength").default(1), // 1-5 scale based on interactions
  establishedAt: timestamp("established_at").defaultNow(),
  lastInteractionAt: timestamp("last_interaction_at"), // Last meaningful interaction
  isActive: boolean("is_active").default(true), // Whether connection is still active
  notes: text("notes"), // Private notes about the connection
  tags: jsonb("tags").default('[]'), // User-defined tags for organizing connections
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  // Ensure no duplicate connections with canonical ordering (lower ID always as user1)
  uniqueConnection: unique().on(table.user1Id, table.user2Id),
  // Prevent self-connections
  checkNoSelfConnection: check('check_no_self_connection', sql`${table.user1Id} != ${table.user2Id}`),
  // Enforce canonical ordering - user1Id must be less than user2Id
  checkCanonicalOrdering: check('check_canonical_ordering', sql`${table.user1Id} < ${table.user2Id}`),
}));

// Connection recommendations table - stores AI-generated connection suggestions
export const connectionRecommendations = pgTable("connection_recommendations", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(), // User receiving the recommendation
  recommendedUserId: integer("recommended_user_id").references(() => users.id).notNull(), // User being recommended
  recommendationType: text("recommendation_type").notNull(), // "smart_connect", "industry_match", "skill_complement", etc.
  matchScore: decimal("match_score", { precision: 3, scale: 2 }).notNull(), // Overall compatibility score (0.00-1.00)
  industryMatchScore: decimal("industry_match_score", { precision: 3, scale: 2 }), // Industry alignment score
  skillMatchScore: decimal("skill_match_score", { precision: 3, scale: 2 }), // Skill compatibility score  
  locationMatchScore: decimal("location_match_score", { precision: 3, scale: 2 }), // Location proximity score
  interestMatchScore: decimal("interest_match_score", { precision: 3, scale: 2 }), // Interest alignment score
  matchReasons: jsonb("match_reasons").default('[]'), // Array of specific reasons for the match
  strengthAreas: jsonb("strength_areas").default('[]'), // Top areas of compatibility
  compatibilityInsights: jsonb("compatibility_insights").default('[]'), // Human-readable insights
  recommendationMetadata: jsonb("recommendation_metadata").default('{}'), // Detailed matching data
  isViewed: boolean("is_viewed").default(false), // Whether user has seen this recommendation
  isDismissed: boolean("is_dismissed").default(false), // Whether user dismissed this recommendation
  wasConnectionRequested: boolean("was_connection_requested").default(false), // Whether user sent a connection request
  generatedAt: timestamp("generated_at").defaultNow(),
  expiresAt: timestamp("expires_at"), // When recommendation expires (optional)
  viewedAt: timestamp("viewed_at"), // When user first viewed this recommendation
  dismissedAt: timestamp("dismissed_at"), // When user dismissed this recommendation
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Insert schemas for Smart Connect
export const insertConnectionRequestSchema = createInsertSchema(connectionRequests).omit({
  id: true,
  status: true,
  requestedAt: true,
  respondedAt: true,
  connectionEstablishedAt: true,
...
export const insertConnectionRecommendationSchema = createInsertSchema(connectionRecommendations).omit({
  id: true,
  isViewed: true,
  isDismissed: true,
...
// Export types for Smart Connect
...
export type ConnectionRecommendation = typeof connectionRecommendations.$inferSelect;
export type InsertConnectionRecommendation = z.infer<typeof insertConnectionRecommendationSchema>;
