...
// FIX: If module resolution fails, we'll try using commonjs require() as a fallback
let dbPool;
try {
  dbPool = pool; // Use the imported pool if available
} catch (err) {
  if (err.code === 'ERR_MODULE_NOT_FOUND') {
    console.log('ESM import failed, falling back to CommonJS require()');
    // Fallback to CommonJS
    const { pool: cjsPool } = require('./server/db');
    dbPool = cjsPool;
  } else {
    throw err;
  }
