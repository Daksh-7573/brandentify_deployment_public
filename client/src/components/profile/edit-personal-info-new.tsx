...
const EditPersonalInfoNew: React.FC<EditPersonalInfoProps> = ({ userData, onCancel, onSave }) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[FORM] Form submitted directly");
    ...
    console.log("[FORM] Making API call to:", `/api/users/${userData.id}`);
    ...
    try {
      ...
      onSave();
      console.log("[FORM] Profile save completed successfully");
    } catch (error) {
      console.error('[FORM] Error updating profile:', error);
      toast({
        title: "Update Failed",
        description: "Failed to update profile. Please try again.",
        variant: "destructive",
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-6 p-6 neo-glass-card backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-2xl shadow-2xl">
      {/* Header */}
