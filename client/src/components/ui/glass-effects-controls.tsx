...
export const GlassEffectsControls: React.FC = () => {
  ...
  // Ensure we use valid props for the GlassCard in the main controls
  const safeBackgroundEffectMain = 
    settings.backgroundEffect === 'none' || 
    settings.backgroundEffect === 'gradient' || 
    settings.backgroundEffect === 'noise' || 
    settings.backgroundEffect === 'glow' 
    ? settings.backgroundEffect 
    : 'none';

  const safeBackgroundIntensityMain = 
    settings.backgroundIntensity === 'low' || 
    settings.backgroundIntensity === 'medium' || 
    settings.backgroundIntensity === 'high' 
    ? settings.backgroundIntensity 
    : 'medium';

  return (
    <AnimatePresence>
      {settings.isControlsOpen && (
        <motion.div 
        ...
        >
          ...
          >
            <GlassCard
              ...
              blurStrength={settings.blurStrength as any}
              transparency={settings.transparency as any}
              backgroundEffect={safeBackgroundEffectMain}
              backgroundIntensity={safeBackgroundIntensityMain}
...
export const GlassEffectsToggle: React.FC = () => {
  const { toggleControls, settings } = useGlassEffects();

  // Ensure we use valid props for the GlassCard
  const safeBackgroundEffect = 
    settings.backgroundEffect === 'none' || 
    settings.backgroundEffect === 'gradient' || 
    settings.backgroundEffect === 'noise' || 
    settings.backgroundEffect === 'glow' 
  ...
  return (
    ...
    >
      ...
      >
        <motion.button
          className="relative flex items-center justify-center p-3 gap-2 text-sm font-medium"
