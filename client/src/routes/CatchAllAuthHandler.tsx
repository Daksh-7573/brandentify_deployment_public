...
/**
 * Special component to handle any possible auth redirect endpoints
 * This component will attempt to process authentication on any URL it's mounted on
 */
export default function CatchAllAuthHandler() {
  ...
  useEffect(() => {
    const handleRedirect = async () => {
      try {
        console.log("CatchAllAuthHandler: Processing redirect result at", window.location.pathname);
        
        // Get the redirect result
        const result = await getRedirectResult(auth);
        
        if (result) {
          console.log("CatchAllAuthHandler: Redirect result found", result.user.uid);
          ...
          // Redirect to home after successful login with a slight delay
          setTimeout(() => {
            navigate('/');
          }, 500);
        } else {
          console.log("CatchAllAuthHandler: No redirect result found");
        ...
        console.error("CatchAllAuthHandler Error:", err);
    ...
    // Execute immediately
    handleRedirect();
  }, [refreshUserData, toast, navigate]);
