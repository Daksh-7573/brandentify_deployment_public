                  ...
                  {!submittedQuery ? (
                    ...
                    <div className="space-y-4">
                      {searchResults.pulses.map((pulse) => (
                        <Card key={pulse.id} className="bg-gray-900/60 backdrop-blur-md border border-white/10 shadow-xl hover:shadow-2xl transition-all hover:scale-[1.01]">
                          <CardContent className="p-4">
                            <div className="flex items-center gap-3 mb-3">
                              <Avatar className="border border-white/10">
                                <AvatarImage src={pulse.user?.photoURL || undefined} />
                                <AvatarFallback className="bg-white/10 text-white">
                                  {getInitials(pulse.user?.name || 'User')}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                                <p className="font-medium text-white">{pulse.user?.name}</p>
                                <p className="text-xs text-gray-300">{formatDistanceToNow(new Date(pulse.createdAt), { addSuffix: true })}</p>
                              </div>
                            </div>
                            <h3 className="text-lg font-medium text-white mb-2">{pulse.title}</h3>
                            <p className="text-gray-300 line-clamp-3">{pulse.content}</p>
                            <div className="flex items-center gap-1 mt-4 text-xs text-gray-400">
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white/10">
                                {pulse.type === 'poll' && <MessageSquare size={12} />}
                                {pulse.type === 'media-pulse' && <Image size={12} />}
                                {pulse.type === 'project' && <FileCode size={12} />}
                                {pulse.type}
                              </span>
                            </div>
                  ...
                  <Card className="bg-gray-900/60 backdrop-blur-md border border-white/10 shadow-xl">
                    ...
                    <CardContent>
                      {matchMutation.isPending ? (
                        ...
                        <div className="text-center py-12 border border-white/10 rounded-lg bg-gray-900/60 backdrop-blur-md">
                          <UserPlus className="mx-auto h-12 w-12 text-white/70 mb-4" />
