...
## Recent Changes
- **October 23, 2025**: Implemented intelligent quest assignment algorithm that assigns 1-4 quests per day based on time budget (60-min daily cap) and XP optimization. System generates 4 candidate quests (2 career + 2 social), calculates estimated time from subtasks, computes XP-per-minute efficiency and impact scores (XP/min × difficulty multiplier), then selects optimal combination using greedy algorithm. Prioritizes high-impact quests while respecting time constraints. Example: 55-min high-XP quest assigned solo, or 3 smaller quests (20+15+20 min) to maximize XP within 60-min budget.
