...
- **Profile URL Random Link Priority**: Fixed Quantum Card and Personal Info Section to prioritize permanent `randomProfileLink` URLs (e.g., `/r/def456ghi789`) over brand name URLs. Updated both components to: (1) Use `randomProfileLink` if it exists, creating stable `/r/{randomLink}` URLs, (2) Fall back to `/@brandname` or `/@username` if no random link exists, (3) Made Quantum Card profile URL clickable (was display-only text before). This ensures all users have consistent, permanent profile URLs that don't break when brand names change. Random profile links are auto-generated at account creation via `ensureUniqueRandomProfileLink()`.
- **Profile URL Routing Fix**: Created ProfileResolver component that uses `/api/users/brand/:identifier` endpoint (queries both brand_name and username with priority for brand_name), renders BrandProfile when identifier matches brand_name, otherwise renders PublicProfile. Removed duplicate routes, keeping single `/@:identifier` route. Both `/@brandname` and `/r/randomlink` URLs now work correctly.
